...
export class MatchService {
  ...
  protected convertDateFromClient(match: IMatch): IMatch {
    const copy: IMatch = Object.assign({}, match, {
      timeOfMessage: match.timeOfMessage && match.timeOfMessage.isValid() ? match.timeOfMessage.toJSON() : undefined,
      matchDate: match.matchDate && match.matchDate.isValid() ? match.matchDate.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      ...
      res.body.matchDate = res.body.matchDate ? moment(res.body.matchDate) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((match: IMatch) => {
        match.timeOfMessage = match.timeOfMessage ? moment(match.timeOfMessage) : undefined;
        match.matchDate = match.matchDate ? moment(match.matchDate) : undefined;
      });
    }
    return res;
  }
