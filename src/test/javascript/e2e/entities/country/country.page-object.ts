...
export class CountryUpdatePage {
  ...
  langageInput = element(by.id('field_langage'));
  ...
  async setLangageInput(langage: string): Promise<void> {
    await this.langageInput.sendKeys(langage);
  }

  async getLangageInput(): Promise<string> {
    return await this.langageInput.getAttribute('value');
  }
